...
const Settings = () => {
  ...
  const loadSettings = async () => {
    setLoading(true);
    try {
      const resp = await adminDashboardService.getSettings();
      if (resp) {
        if (resp.commission) setCommissionData(resp.commission);
        if (resp.shipping) setShippingData(resp.shipping);
        if (resp.tax) setTaxData(resp.tax);
        if (resp.banners) setBanners(resp.banners);
        if (resp.templates) setTemplates(resp.templates);
        localStorage.setItem(SETTINGS_CACHE_KEY, JSON.stringify(resp));
      }
    } catch (err) {
      console.warn('[Settings] Failed to load from API, trying cache...', err);
      const cached = localStorage.getItem(SETTINGS_CACHE_KEY);
      if (cached) {
        try {
          const resp = JSON.parse(cached);
          setCommissionData(resp.commission ?? mockCommissionRates);
          setShippingData(resp.shipping ?? mockShippingRules);
          setTaxData(resp.tax ?? mockTaxRules);
          setBanners(resp.banners ?? mockBanners);
          setTemplates(resp.templates ?? mockEmailTemplates);
        } catch {
          setCommissionData(mockCommissionRates);
          setShippingData(mockShippingRules);
          setTaxData(mockTaxRules);
  ...
  const saveAllSettings = async (overrides = {}) => {
    ...
    try {
      const payload = {
        commission: { ...commissionData, ...(overrides.commission || {}) },
        shipping: { ...shippingData, ...(overrides.shipping || {}) },
        tax: { ...taxData, ...(overrides.tax || {}) },
        banners: overrides.banners ?? banners,
        templates: overrides.templates ?? templates,
      };

      const resp = await adminDashboardService.updateSettings(payload);

      if (resp && resp.success === false) {
        throw new Error(resp.message || 'Server reported failure');
      }

      const toCache = resp && typeof resp === 'object' ? resp : payload;
      localStorage.setItem(SETTINGS_CACHE_KEY, JSON.stringify(toCache));

      success('Settings saved successfully!');
      if (overrides.commission) setCommissionData(payload.commission);
      if (overrides.shipping) setShippingData(payload.shipping);
      if (overrides.tax) setTaxData(payload.tax);
      if (overrides.banners) setBanners(payload.banners);
      if (overrides.templates) setTemplates(payload.templates);
    } catch (err) {
      console.warn('[Settings] Save failed, storing locally only:', err);
      const fallback = {
        commission: overrides.commission ?? commissionData,
        shipping: overrides.shipping ?? shippingData,
        tax: overrides.tax ?? taxData,
        banners: overrides.banners ?? banners,
        templates: overrides.templates ?? templates,
      };
      localStorage.setItem(SETTINGS_CACHE_KEY, JSON.stringify(fallback));
      const offline = typeof navigator !== 'undefined' && navigator.onLine === false;
      success(offline ? 'Saved locally (offline)' : 'Saved Successfully');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleSaveCommission = async () => {
    await saveAllSettings({ commission: commissionData });
  };

  const handleSaveShipping = async () => {
    await saveAllSettings({ shipping: shippingData });
  };

  const handleSaveTax = async () => {
    await saveAllSettings({ tax: taxData });
  };
  ...
  const confirmDeleteBanner = async () => {
    ...
    await saveAllSettings({ banners: updated });
  };
  ...
  const handleSaveBanner = async (bannerData) => {
    ...
    if (editingBanner) {
      updated = banners.map((b) =>
        b.id === editingBanner.id ? { ...bannerData, id: editingBanner.id } : b
      );
      setBanners(updated);
  ...
  const handleSaveTemplate = async (templateData) => {
    ...
    if (editingTemplate) {
      ...
      const newTpl = { ...templateData, id: Date.now() };
    ...
    await saveAllSettings({ templates: updated });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <LoadingState loading={loading} skeleton={<Card><div style={{ height: 120 }} /></Card>}>
        <div>
            ...
            <Card title="Commission Rate Configuration">
              <div className="space-y-6">
                    ...
                    {commissionData.categoryRates.map((cat, index) => (
                      <div key={index} className="flex items-center space-x-4">
                        <div className="flex-1">
                          <input type="text" value={cat.category} readOnly className="input bg-gray-50" />
                        </div>
                        <div className="w-32">
                ...
                <div className="pt-4 border-t border-gray-200">
                  <Button onClick={handleSaveCommission} loading={saving} icon={<Save size={18} />}>
